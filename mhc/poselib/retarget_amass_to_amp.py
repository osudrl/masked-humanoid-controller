...
def generate_smpl_zero_pose():
    xml_path = "mhc/data/assets/smpl_humanoid.xml"
    skeleton = SkeletonTree.from_mjcf(xml_path)
    zero_pose = SkeletonState.zero_pose(skeleton)
    zero_pose.to_file('smpl_tpose.npy')
...
if __name__ == '__main__':
    '''
    To run:
    1. Process AMASS dataset to obtain motions on smpl_humanoid: `python ase/poselib/process_amass.py`
    ...
    amass_data_dir = home_folder+"/Downloads/amass_processed"
    amass_output_dir = home_folder+"/Downloads/amass_processed_retargeted"
    amass_occlusion = joblib.load("/home/pan/ase/ase/data/amass/amass_copycat_occlusion_v2.pkl")
    all_motions = []
        ...
        for root, dirs, files in os.walk(curr_data_dir):
            for file in sorted(files):
                if file.endswith(".npy"):
                    ...
                    if '0-'+dataset+"_"+root.replace(curr_data_dir,'')[1:]+'_'+file[:-4] not in amass_occlusion:
                        all_motions.append({"file": target_file.replace(amass_output_dir+"/", ""), "weight": 1.0, "num_frames": num_frames})
                        curr_dataset_motions.append({"file": target_file.replace(os.path.join(amass_output_dir,dataset)+"/", ""), "weight": 1.0, "num_frames": num_frames})
